...
import numpy as np
import torch
import torch.distributions as td # Getting the distributions from torch
import torch.nn as nn # Building the NN

class MoG(nn.Module): # Mixture of Gaussians, Needs the means and covariances
    def __init__(self, means, covars, weights=None,
        ...
        self.means = means        # Assigning means and covariances
        self.covars = covars
        ...
        # Determinant covariances
        self.dets = torch.stack([torch.det(cv) for cv in covars])

    # Sampling
    def sample(self, n):
        # Generates n samples from the cs_distribution (associated to weights)
        cs = self.cs_distrib.sample_n(n).to(self.device)
        
        # Tensor that holds all the sample, for each distribution
