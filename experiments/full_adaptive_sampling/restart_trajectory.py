...
xs_, accs_, us_, counts_ = run_metropolis(model=mixture, u_init=uis, x_init=xis, count_init=cis, 
                                            n_sample=n_chains, n_steps=n_sts, mixture=True)
...
for i in range(n_runs):
    ...
    mpi.world.barrier()
    if ((i+1)%2==0):
        md_data = run_md_get_zmat(ag6, md_iters, name+'_'+str(i+1), starting=False)
